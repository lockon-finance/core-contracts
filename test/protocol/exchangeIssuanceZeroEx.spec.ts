/// Created based on index-coop-smart-contracts
/// https://github.com/IndexCoop/index-coop-smart-contracts/blob/master/test/exchangeIssuance/exchangeIssuanceZeroEx.spec.ts
...
describe("ExchangeIssuanceZeroEx [ @forked-mainnet ]", async () => {
  let owner: Account;
  let user: Account;
  ...
  describe("#constructor", async () => {
    ...
    async function subject(): Promise<ExchangeIssuanceZeroEx> {
      return await deployer.core.deployExchangeIssuanceZeroEx(
        subjectWethAddress,
  ...
  context("when exchange issuance is deployed", async () => {
    ...
    cacheBeforeEach(async () => {
      ...
      exchangeIssuanceZeroEx = await deployer.core.deployExchangeIssuanceZeroEx(
        wethAddress,
        controllerAddress,
        zeroExMock.address,
      );
      ...
      context(`when issuance module is ${issuanceModuleName}`, () => {
        ...
        describe("#issueExactSetFromETH", async () => {
          ...
          it("emits an ExchangeIssue log", async () => {
            ...
            const isEmitExchangeIssue = receipt.events?.some(
              event => event.event === "ExchangeIssue",
            );
            expect(isEmitExchangeIssue).to.eq(true);
            ...
            expect(
              finalBalanceOfSet).to.eq(expectedBalance);
          });
